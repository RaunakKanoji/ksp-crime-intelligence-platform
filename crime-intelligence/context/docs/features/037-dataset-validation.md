...
## 1. Summary
Validate uploaded crime datasets. This feature is part of the `Data Operations` group and must be implemented only when it is the active feature. It helps the KSP Crime Intelligence web app support secure, explainable, and progressive crime intelligence workflows without turning the project into a multi-app or mobile implementation.

## 2. Problem
Current crime analysis workflows can be fragmented, manual, and dependent on spreadsheet-heavy processes. Without a focused dataset validation feature, users may lack a reliable way to perform this workflow with proper security, data context, permission controls, and auditability.
...
## 4. Goals
- Deliver Dataset Validation in a focused, maintainable way.
...
## 6. User Stories
- As a data officer, I want to use dataset validation so that I can complete my crime-intelligence task without switching to manual spreadsheets.
...
- As an admin/security reviewer, I want dataset validation to respect permissions and audit requirements so that sensitive crime data remains protected.

## 7. Functional Requirements
- Provide the core workflow for dataset validation exactly within the active feature scope.
- Support Required columns.
- Support Data type checks.
- Support Date parsing.
- Support Duplicate detection.
- Support Missing value report.
- Support Invalid location report.
- Support Legal section validation.
...
- The page or component title should clearly read `Dataset Validation`.
...
- UI should account for required columns.
- UI should account for data type checks.
- UI should account for date parsing.
- UI should account for duplicate detection.
- UI should account for missing value report.
...
- Consider data needed for required columns.
- Consider data needed for data type checks.
- Consider data needed for date parsing.
- Consider data needed for duplicate detection.
- Consider data needed for missing value report.
...
- Validate inputs related to required columns.
- Validate inputs related to data type checks.
- Validate inputs related to date parsing.
...
- A user with the correct permission can access dataset validation.
...
- The implementation supports required columns.
- The implementation supports data type checks.
- The implementation supports date parsing.
- The implementation supports duplicate detection.
- The implementation supports missing value report.
...
- Catalyst-first: Run validation in Catalyst server-side logic. Store validation status and invalid-row reports through Catalyst Data Store and Stratus.
- Implement Dataset Validation in isolation after reading this spec and the main project docs.
