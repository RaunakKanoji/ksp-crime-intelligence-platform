# Feature 045: Property Offence Analysis

## 1. Summary
Analyze theft, burglary, robbery, and property-related offences. This feature is part of the `Domain Analytics` group and must be implemented only when it is the active feature. It helps the KSP Crime Intelligence web app support secure, explainable, and progressive crime intelligence workflows without turning the project into a multi-app or mobile implementation.

## 2. Problem
Current crime analysis workflows can be fragmented, manual, and dependent on spreadsheet-heavy processes. Without a focused property offence analysis feature, users may lack a reliable way to perform this workflow with proper security, data context, permission controls, and auditability.
...
## 4. Goals
- Deliver Property Offence Analysis in a focused, maintainable way.
...
- Support stolen property type.
- Support estimated value if available.
...
## 6. User Stories
- As a crime analyst, I want to use property offence analysis so that I can complete my crime-intelligence task without switching to manual spreadsheets.
...
- As an admin/security reviewer, I want property offence analysis to respect permissions and audit requirements so that sensitive crime data remains protected.

## 7. Functional Requirements
- Provide the core workflow for property offence analysis exactly within the active feature scope.
- Support Category filters.
- Support Stolen property type.
- Support Estimated value if available.
- Support Recovery status.
- Support Hotspots.
- Support Repeat offenders.
- Support Time trends.
...
- The page or component title should clearly read `Property Offence Analysis`.
...
- UI should account for stolen property type.
- UI should account for estimated value if available.
- UI should account for recovery status.
- UI should account for hotspots.
...
- Consider data needed for stolen property type.
- Consider data needed for estimated value if available.
- Consider data needed for recovery status.
- Consider data needed for hotspots.
...
- Potential route or service function may be added for `property-offence-analysis` if required by implementation.
...
- Validate inputs related to stolen property type.
- Validate inputs related to estimated value if available.
...
- A user with the correct permission can access property offence analysis.
...
- The implementation supports stolen property type.
- The implementation supports estimated value if available.
- The implementation supports recovery status.
- The implementation supports hotspots.
...
- Implement Property Offence Analysis in isolation after reading this spec and the main project docs.
