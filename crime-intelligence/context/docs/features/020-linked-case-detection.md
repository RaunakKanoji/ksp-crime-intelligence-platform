...
## 1. Summary
Detect possible links between cases. This feature is part of the `Intelligence` group and must be implemented only when it is the active feature. It helps the KSP Crime Intelligence web app support secure, explainable, and progressive crime intelligence workflows without turning the project into a multi-app or mobile implementation.

## 2. Problem
Current crime analysis workflows can be fragmented, manual, and dependent on spreadsheet-heavy processes. Without a focused linked case detection feature, users may lack a reliable way to perform this workflow with proper security, data context, permission controls, and auditability.
...
## 4. Goals
- Deliver Linked Case Detection in a focused, maintainable way.
...
- Support same accused signal.
- Support same vehicle signal.
- Support similar modus operandi signal.
- Support same location signal.
...
## 6. User Stories
- As an investigator, I want to use linked case detection so that I can complete my crime-intelligence task without switching to manual spreadsheets.
...
- As an admin/security reviewer, I want linked case detection to respect permissions and audit requirements so that sensitive crime data remains protected.

## 7. Functional Requirements
- Provide the core workflow for linked case detection exactly within the active feature scope.
- Support Same accused signal.
- Support Same vehicle signal.
- Support Similar modus operandi signal.
- Support Same location signal.
- Support Same phone number if permitted.
- Support Similar time pattern.
- Support Same property type.
- Support Shared associates.
...
- The page or component title should clearly read `Linked Case Detection`.
- Show clear labels, helper text, and safe empty states.
- UI should account for same accused signal.
- UI should account for same vehicle signal.
- UI should account for similar modus operandi signal.
- UI should account for same location signal.
- UI should account for same phone number if permitted.
...
- Consider data needed for same accused signal.
- Consider data needed for same vehicle signal.
- Consider data needed for similar modus operandi signal.
- Consider data needed for same location signal.
- Consider data needed for same phone number if permitted.
...
- Potential route or service function may be added for `linked-case-detection` if required by implementation.
...
- Validate inputs related to same accused signal.
- Validate inputs related to same vehicle signal.
- Validate inputs related to similar modus operandi signal.
...
- A user with the correct permission can access linked case detection.
...
- The implementation supports same accused signal.
- The implementation supports same vehicle signal.
- The implementation supports similar modus operandi signal.
- The implementation supports same location signal.
- The implementation supports same phone number if permitted.
...
- Implement Linked Case Detection in isolation after reading this spec and the main project docs.
